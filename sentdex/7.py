...
class DabsonBot(sc2.BotAI):
  ...
  async def intel(self):
    #print(dir(self))
    #self.f.write(''.join(dir(self))+'\n') #for some reason, writes in a dir above
    #self.game_info.map_size
    #in the array, its height by width
    game_data = np.zeros((self.game_info.map_size[1], self.game_info.map_size[0], 3), np.uint8)
    for nexus in self.units(NEXUS):
      nex_pos = nexus.position

      #data, coord coord, px_size, color, line_width(-1 for no stroke)
